...
function StatsPage() {
  ...
  const botWins = matches?.filter((m) => m.result === "win" && m.difficulty !== "player").length ?? 0;
  const pvpWins = matches?.filter((m) => m.result === "win" && m.difficulty === "player").length ?? 0;

  function difficultyLabel(d: string) {
    if (d === "player") return "vs Player";
    return `vs Bot (${d.charAt(0).toUpperCase() + d.slice(1)})`;
  }
    ...
    <div className="p-6 lg:p-12 max-w-5xl mx-auto">
      ...
      <div className="grid grid-cols-2 md:grid-cols-3 lg:grid-cols-6 gap-4 mb-8">
        {[
          ...
          { l: "Bot Wins", v: botWins },
          { l: "PvP Wins", v: pvpWins },
        ].map((s) => (
              ...
              <tbody>
                {matches.map((m) => (
                  <tr key={m.id} className="border-b border-border/20">
                    ...
                    <td className="text-xs">{difficultyLabel(m.difficulty)}</td>
                    <td>{m.shots_fired}</td>
